...
veryOldInventors = inventors.filter((born) => {
  return 1500 <= born.year && born.year <= 1599
})
...
inventorNames = inventors.map(({ first, last }) => ({ first: first, last: last }))
...
let firstLast = []
let nameM,nameM2,nameF,nameL,orderedName;
people.map((name) => {
  nameM = name.split(' , ');
// console.log("NameM:"+nameM);
nameM.forEach(fullName=>{
  // console.log("full name"+fullName);
nameM2 = fullName.split(", ");
nameF = nameM2[1];
nameL = nameM2[0];
// console.log("full name in order: "+nameF+" "+nameL);
orderedName = nameF+" "+nameL;
  // console.log("full name in order: "+orderedName);
firstLast.push(orderedName);
...
totalYearsLived = inventors.reduce((total, inventor)=>{
  let totalLive = inventor.passed - inventor.year 
  return total + totalLive
...
travelMethodCounts = travelMethods.reduce((total, travel)=>{
  if (total[travel]) {
    total[travel] += 1;
  } else {
    total[travel] = 1;
